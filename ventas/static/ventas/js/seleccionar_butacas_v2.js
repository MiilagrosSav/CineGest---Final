// Butacas selection logic - VERSION 5.0 (2x1 MEJORADO)
...
console.log('✅ [v5.0] Script cargado. Precio unitario:', precioUnitario, 'Tipo descuento:', tipoDescuento, 'Requiere 4D:', requiere4D);
...
function actualizarResumen() {
    ...
    // ✅ CORRECCIÓN: precioUnitario YA viene con descuento aplicado desde el backend
    let total = 0;
    let detallePromocion = '';
    
    if (tipoDescuento === '2X1') {
        // Para 2x1: cobrar solo la mitad de las entradas
        const entradasAPagar = Math.ceil(cantidad / 2);
        total = entradasAPagar * precioUnitario;
        
        // Mostrar claramente cuántas pagás y cuántas llevás
        if (cantidad > 0) {
            detallePromocion = `🎉 Pagás ${entradasAPagar} y llevás ${cantidad}`;
        }
    } else {
        // Para otros descuentos (PORCENTAJE, MONTO_FIJO) o sin descuento:
        // precioUnitario ya incluye el descuento, solo multiplicar
        total = cantidad * precioUnitario;
    }
    
    console.log('Actualizando resumen. Cantidad:', cantidad, 'Total:', total, 'Tipo descuento:', tipoDescuento, 'Precio unitario:', precioUnitario);
    
    // Actualizar cantidad con detalle de promoción si aplica
    const cantidadText = cantidad + (detallePromocion ? ` (${detallePromocion})` : '');
    document.getElementById('cantidadButacas').innerHTML = detallePromocion 
        ? `${cantidad}<br><small style="color: var(--accent); font-size: 0.75rem; font-weight: normal;">${detallePromocion}</small>`
        : cantidad;
    ...
    if (cantidad === 0) {
        ...
        butacasSeleccionadas.forEach((butaca, idx) => {
            // ✅ CORRECCIÓN: Calcular precio por butaca según tipo de descuento
            let precio = precioUnitario;
            let precioTexto = `$${precio.toFixed(2)}`;
            
            if (tipoDescuento === '2X1') {
                // En 2x1: las impares (idx 1, 3, 5...) son gratis
                if (idx % 2 === 1) {
                    precio = 0;
                    precioTexto = 'GRATIS 🎉';
                }
            ...
            if (cantidadNecesaria) {
                ...
                html += `<div style="background: rgba(0,0,0,0.3); padding: 0.5rem; border-radius: 6px; display: flex; justify-content: space-between; align-items: center;">
                    <span>💺 Butaca ${butaca.fila}${butaca.numero}</span>
                    <span style="color: ${precio === 0 ? 'var(--accent)' : 'var(--primary)'}; font-weight: 600;">${precioTexto}</span>
                </div>`;
            }
        });
        html += '</div>';
