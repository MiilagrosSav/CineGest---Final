...
from cine.models import Sala, Pelicula
...
from reportes.utils import (
    ...
    parse_report_filters_from_request,
...
def reporte_financiero_view(request):
    ...
    quick_ranges = get_quick_range_querystrings(extra_params=filters)

    kpis = get_kpis(start_dt, end_dt, filters=filters)
    fin_data = get_datos_financieros(start_dt, end_dt, limit=config.LIMIT_DEFAULT, filters=filters)
    ...
    ranking_dia_revenue = get_ranking_dia_revenue(start_dt, end_dt, filters=filters)  # Ya viene ordenado por revenue descendente
    analisis_franjas = get_analisis_franjas_horarias(start_dt, end_dt, filters=filters)
    ...
    context = {
        ...
        **quick_ranges,
        ...
        'salas_filtro': salas,
        'metodos_pago_filtro': metodos_pago,
        'estados_venta_filtro': [estado[0] for estado in Venta.ESTADO_CHOICES],
        'estados_pago_filtro': [estado[0] for estado in Pago.ESTADO_CHOICES],
    }
...
def reporte_ocupacion_view(request):
    ...
    heatmap = get_ocupacion_por_franja_horaria(start_dt, end_dt, filters=filters)
...
@login_required
def buscar_peliculas_ajax(request):
    """Devuelve sugerencias de títulos para autocompletar filtros de reportes."""
    if not is_system_admin(request.user):
    ...
    peliculas = (
        ...
        .order_by('titulo')
        .values_list('titulo', flat=True)[:10]
    )
    return JsonResponse({'results': list(peliculas)})


__all__ = ["reporte_financiero_view", "reporte_ocupacion_view", "buscar_peliculas_ajax"]
