// JS extracted from templates/cine/sala_list.html
var selectedRows = [];
...
function attachRowListeners(container) {
    if (!container) return;
    const rows = container.querySelectorAll('.sala-row');
    rows.forEach(row => {
        row.addEventListener('click', function(event) {
            if (event.target.classList.contains('btn-layout')) { return; }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(btnModificar) btnModificar.addEventListener('click', function() {
        if (selectedRows.length === 1) {
            window.location.href = `/salas/${selectedRows[0]}/editar/`;
        }
    });
    if(btnEliminar) btnEliminar.addEventListener('click', function() {
        if (selectedRows.length === 0) return;
        if (selectedRows.length === 1) {
            window.location.href = `/salas/${selectedRows[0]}/eliminar/`;
        } else {
...
// AJAX pagination handling (same pattern)
(function(){
